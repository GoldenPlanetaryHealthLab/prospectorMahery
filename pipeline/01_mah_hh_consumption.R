...
#============== (1) Read in data ===============================================
library(tidyverse)        # for tidy data routines #TODO slim this down
# library(magrittr)         # for efficient piping Not necessary if tidyverse is loaded
library(readxl)           # for reading excel files

# BAD. Don't do this. Use here()
...
# also bad. Use devtools::load_all(".")
# source(paste0(wd, code_dir, "cleaning_functions.R"))
devtools::load_all(".")  # Load all functions in the package (assuming this code is part of a package)
...
if(!file.exists(mah_consump)) {
  stop(paste0("File ", mah_consump, " does not exist. Please check the file path and name."))
}
mah_hh_total <- read.csv(
  external_in(
    "/n/holylabs/cgolden_lab/Lab/frontier/works/prospectors/prospectorMahery/data/inputs/01_gold_mine/madagascar-cohort-data-zip/mah_hh_dietary_intake.csv"
    )
  )

# Read in MAHERY metadata
mah_meta_f <- here("data", "inputs", "01_gold_mine", "madagascar-cohort-data-zip", "mahery_metadata.xlsx")
if(!file.exists(mah_meta_f)) {
  stop(paste0("File ", mah_meta_f, " does not exist. Please check the file path and name."))
}
mah_meta <- read_excel(
  external_in(
    "/n/holylabs/cgolden_lab/Lab/frontier/works/prospectors/prospectorMahery/data/inputs/01_gold_mine/madagascar-cohort-data-zip/mahery_metadata.xlsx"
    ),
    sheet = 2
  )
...
food_cats <- here("data", "inputs", "01_gold_mine", "madagascar-cohort-data-zip", "MAHERY_DARWIN_ForMatching_AllUniqueFoods_to_Nutritional_Equivalents2025Feb1.xlsx")
if(!file.exists(food_cats)) {
  stop(paste0("File ", food_cats, " does not exist. Please check the file path and name."))
}
new_groups <- read_xlsx(
  external_in(
    "/n/holylabs/cgolden_lab/Lab/frontier/works/prospectors/prospectorMahery/data/inputs/01_gold_mine/madagascar-cohort-data-zip/MAHERY_DARWIN_ForMatching_AllUniqueFoods_to_Nutritional_Equivalents2025Feb1.xlsx"
    ),
...
food_groupings <- here("data", "inputs", "01_gold_mine", "madagascar-cohort-data-zip", "dar_hh_weekly_per_month_grams_categs_long.csv")
if(!file.exists(food_groupings)) {
  stop(paste0("File ", food_groupings, " does not exist. Please check the file path and name."))
}
hh_comb_groups <- read.csv(
  external_in(
    "/n/holylabs/cgolden_lab/Lab/frontier/works/prospectors/prospectorMahery/data/inputs/01_gold_mine/madagascar-cohort-data-zip/dar_hh_weekly_per_month_grams_categs_long.csv"
    )
  )

# Read in fish encyclopedia
fish_encyclo <- here("data", "inputs", "01_gold_mine", "madagascar-cohort-data-zip", "fish_enc_17Feb2025.csv")
if(!file.exists(fish_encyclo)) {
  stop(paste0("File ", fish_encyclo, " does not exist. Please check the file path and name."))
}
fish_enc_g <- read.csv(
  external_in(
    "/n/holylabs/cgolden_lab/Lab/frontier/works/prospectors/prospectorMahery/data/inputs/01_gold_mine/madagascar-cohort-data-zip/fish_enc_17Feb2025.csv"
    )
  ) %>% 
